...
## 24. 买家端原始 HTML 是打包压缩格式（2026-05-04 session 2）
- 问题：买家端所有页面原始备份是打包格式，内容编码压缩
- 影响：无法用 PowerShell debug 脚本读取元素 id 和 class
- 正确做法：用浏览器 F12 → 元素标签查看渲染后的真实结构
- 教训：遇到打包格式页面，不要用 print(repr()) 读原始文件，直接看浏览器

## 25. 买家登录链接拦截失败（2026-05-04 session 2）
- 问题：原始 HTML 里 Log In 按钮是 <a href="dashboard.html">，点击直接跳转绕过我们的逻辑
- 失败方法1：MutationObserver 绑定 — 太慢
- 失败方法2：setInterval 每200ms检查 — 太慢
- 失败方法3：读取可见 input — 逻辑对但拦截未生效
- 正确方案：document.addEventListener('click', handler, true) capture phase
  - true 参数让监听器在冒泡之前触发，能在链接跳转前拦截
  - 判断 e.target.closest('a').href.includes('dashboard.html')
  - 调用 e.preventDefault() + e.stopPropagation() 阻止跳转
  - 然后执行 doLogin()
- 下次直接用这个方案，不要再试其他方法
