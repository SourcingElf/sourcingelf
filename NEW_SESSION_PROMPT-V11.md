...
- client.js
- inject_all.py
- suppliers.py（或当天涉及的后端文件）

> project.md 和 schema.md 不需要每次上传，HANDOFF.md 已包含足够信息。
...
**动手前：**
1. 先解释清楚再动手 — 每次给出命令前，必须先用普通语言说清楚"要做什么、为什么、有什么风险"。如果我没有先解释就给出命令，你只需说"先解释"，我必须停下来解释清楚才能继续。
2. 查清楚再动手 — 读取已上传的文件获取实际内容，绝不猜测
3. 修改 inject_all.py 前，必须先用 debug 脚本 print(repr(...)) 确认真实内容和结束标记，不靠 PowerShell 显示判断
...
9. 写任何包含 onclick 的代码，必须先检查引号嵌套再输出 — HTML onclick 属性用双引号，里面字符串必须用 &quot; 而不是单引号。这个错误已经发生过两次，每次都产生 SyntaxError。

**出错时：**
9. 出现 ERROR 先停下 — 分析清楚真正原因才动手，不乱试
10. 不确定就说不确定 — 宁可多问一步，也不冒险硬做

**完成后：**
11. 从用户角度验证 — 想清楚真实用户操作时会经历什么
12. 主动提醒更新交接文档 — 不能只做不记
...
- 修改前必须先用 debug 脚本确认真实内容，见 PROBLEMS.md 第18、21条
...
1. 让 Claude 输出更新后的文件：HANDOFF.md、PROBLEMS.md、NEW_SESSION_PROMPT.md、project.md、project_zh.md
2. 下载并备份以下文件到当天文件夹：

```powershell
mkdir "C:\Projects\更新-2026年x月x日"
Copy-Item "C:\Projects\sourcingelf\HANDOFF.md" "C:\Projects\更新-2026年x月x日\" -Force
Copy-Item "C:\Projects\sourcingelf\PROBLEMS.md" "C:\Projects\更新-2026年x月x日\" -Force
...
Copy-Item "C:\Projects\sourcingelf\routers\suppliers.py" "C:\Projects\更新-2026年x月x日\" -Force
```
