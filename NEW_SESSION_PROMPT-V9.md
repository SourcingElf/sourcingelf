...
2. 查清楚再动手 — 读取已上传的文件（inject_all.py、client.js）获取实际内容，绝不猜测；需要看本地文件时用 Select-String 或 Get-Content 确认
3. 想清楚为什么这样写 — 现有代码可能有其原因，不能凭第一印象认为是错的
4. 想清楚连锁影响 — 这个改动会影响哪些其他页面、功能、业务逻辑
5. 考虑长远延续性 — 这个做法未来加手机版、接入AI时还适用吗？
...
10. 修改 inject_all.py 用行号定位 — 不用字符串匹配（因为文件编码问题会导致 ERROR）

**出错时：**
11. 出现 ERROR 先停下 — 分析清楚真正原因才动手，不乱试
12. 不确定就说不确定 — 宁可多问一步，也不冒险硬做

**完成后：**
13. 从用户角度验证 — 不只看数字对不对，想清楚真实用户操作时会经历什么
14. 发现新教训就记录 — 提醒我更新 PROBLEMS.md，不能只做不记
...
- 这是经过深思熟虑的设计，原因：供应商需要充分了解买家背景才能决定是否花 US$138
...
- 修改 inject_all.py → 运行 inject_all.py → 所有页面更新
- 修改 inject_all.py 用 fix_xxx.py 脚本（行号定位），不手动编辑
