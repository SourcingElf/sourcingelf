...
### 供应商
- `GET /suppliers/me/profile` — 获取自己的资料（含工厂/MOQ/认证/优势/市场）✅ 已测试
...
7. **一个任务可以连接多个供应商**（买家可与多个供应商确认连接）
8. **视频修改**：每个视频最多修改 1 次
9. **法律确认**：供应商必须接受 IP 条款才能发布视频
10. **请求有效期**：30 天
11. **Lead 有效期**：买家自定（最长 6 个月），少于 7 天时显示橙色警告
12. **Credit 确认弹窗**：每次 Connect 操作前必须弹出确认
13. **供应商必须先查看买家资料再决定连接**：
    - Requests 页面卡片上只有 View Profile 按钮
    - 点击后弹窗显示完整遮码买家资料
    - Connect Now 在弹窗内，不在卡片上
    - 原因：供应商需要充分了解买家背景才能决定是否花 US$138
...
- buyer_moqs 表存在于 buyer_profiles，但 Lead Form 买家可能未注册（buyer_id = null）
...
- **Supplier Requests 页面 — 完整重做（2026-05-03）**
  ...
  - Credits 余额检查，0 时禁用按钮（新增）
- **Supplier Connected 页面 — 真实 API 逻辑注入（2026-05-03 晚）**
  - 新增专用端点 GET /suppliers/me/connected-buyers（suppliers.py 末尾）
  - 返回 connection 记录与 buyer_profiles 信息合并，一次调用搞定
  - client.js 新增 SE.SupplierAPI.getConnectedBuyers()
  - 卡片渲染、搜索过滤、Open Chat 按钮全部实现
  - ⚠️ 尚未用真实数据验证——测试账号在 Supabase 没有 connection 记录

### 进行中 ⏳
- Supplier Connected 页面 — 需在 Supabase 插入测试 connection 记录验证卡片渲染
- Buying Leads 页面 — 需要接入真实 API 数据（目前硬编码）
- IM Chat 页面对接
...
| 表 | 记录 | 用途 |
|---|------|------|
| buyer_requests | Sarah Johnson / Anthropologie Group / 美国 / Brand | 测试 Requests 页面 |
